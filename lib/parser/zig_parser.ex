defmodule FnXML.Parser.Zig do
  @moduledoc """
  XML Parser using Zig SIMD scanning.

  Uses Zig NIF for fast character position scanning, then optimized
  Elixir for event classification and extraction.
  """

  alias FnXML.Scanner.Zig, as: Scanner

  @doc """
  Parse XML string into a stream of events.

  Uses SIMD to find all bracket positions, then classifies each element.
  Returns the same event format as FnXML.Parser.parse/1.
  """
  def parse(xml) when is_binary(xml) do
    # Get all element boundaries using SIMD - one NIF call
